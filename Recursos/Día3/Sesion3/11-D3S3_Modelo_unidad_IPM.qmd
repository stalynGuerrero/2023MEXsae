...
$$
logit(\theta_{di}) = \log \left(\frac{\theta_{di}}{1-\theta_{di}}\right) = \eta_{di} =  \boldsymbol{x}_{di}^{T}\boldsymbol{\beta} + u_{d}
$$
...
```{r}
encuesta_ipm <- readRDS("Data/encuestaMEX20_nbi.Rds")
...
```{r,echo=FALSE}
# saveRDS(encuesta_df, "Data/encuesta_df.rds")
encuesta_df <- readRDS("Data/encuesta_df.rds")
...
```{r, eval=FALSE}
fit_agua <-
  readRDS(file = "Data/nbi_agua_ee.rds")
fit_combustible <-
  readRDS(file = "Data/nbi_combus_ee.rds")
fit_techo <-
  readRDS(file = "Data/nbi_techo_ee.rds")
fit_energia <-
  readRDS(file = "Data/nbi_elect_ee.rds")
fit_hacinamiento <-
  readRDS(file = "Data/nbi_hacina_ee.rds")
fit_paredes <-
  readRDS(file = "Data/nbi_pared_ee.rds")
fit_material <-
  readRDS(file = "Data/nbi_matviv_ee.rds")
fit_saneamiento <-
  readRDS(file = "Data/nbi_saneamiento_ee.rds")
fit_piso <-
  readRDS(file = "Data/nbi_piso_ee.rds")
```

Ahora, debemos leer la información del censo  y crear los **post-estrato **
```{r}
censo_ipm <- readRDS("Data/censo_mrp_dam.rds")
...
saveRDS(epred_mat_piso, "Data/epred_mat_piso.rds")
...
saveRDS(epred_mat_piso_dummy, "Data/epred_mat_piso_dummy.rds")
...
chain_q  <-
  (1 / 9) * (
    epred_mat_material_dummy +
      ...
      epred_mat_agua_dummy +
      epred_mat_hacinamiento_dummy)
saveRDS(chain_q, "Data/chain_q.rds")
```
...
```{r}
source("0Recursos//Estimar_ipm.R")
ipm_dam <- estime_IPM(
...
```{r,}
source("0Recursos/agregado_dim_ipm.r")
...
El resultado por dam y para todas las dimensiones es calculando usando el sigueinte código.
...
estimacion_dam_dim <- aux_agregado(epred_mat_dim,
                                   ...
                                   censo = poststrat_df) %>%
  data.frame()
saveRDS(estimacion_dam_dim,"0Recursos/estimacion_dam_dim.Rds")
```



```{r, echo=FALSE}
estimacion_dam_dim <- readRDS("0Recursos/estimacion_dam_dim.Rds")
...
library(sf)
library(tmap)
ShapeSAE <- read_sf("Shape/MEX_dam.shp")  
```
...
knitr::include_graphics("0Recursos/MEX_IPM.jpeg")
```
...
saveRDS(estimacion_dim, "0Recursos/estimacion_dim.rds")
```


```{r, echo=FALSE}
estimacion_dim <- readRDS("0Recursos/estimacion_dim.rds")
```
...
```{r echo=FALSE, out.height="400%",fig.align='center'}
knitr::include_graphics("0Recursos/dim_ipm.png")
```